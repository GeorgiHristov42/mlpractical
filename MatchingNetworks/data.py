...
class OmniglotNShotDataset():
    def __init__(self, batch_size, classes_per_set=10, samples_per_class=1, seed=2591, shuffle_classes=True):
        ...
        self.x = np.load("cifar-100-balanced.npy")
        self.x = np.reshape(self.x, newshape=(100, 300, 32, 32, 3))
        if shuffle_classes:
            class_ids = np.arange(self.x.shape[0])
            np.random.shuffle(class_ids)
            self.x = self.x[class_ids]
        self.x_train, self.x_test, self.x_val = self.x[:80], self.x[80:90], self.x[90:]
